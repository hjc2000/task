...
/// @param func 任务函数
std::shared_ptr<bsp::ITask> bsp::di::task::CreateTask(uint32_t stack_size, std::function<void()> func)
{
	return ::task::FreeTask::Create(func, stack_size);
}

/// @brief 启动调度。本函数会持续阻塞。
void bsp::di::task::StartScheduler()
{
...
void bsp::di::task::SuspendAllTask()
{
	vTaskSuspendAll();
}
...
/// @warning 实现本函数时禁止使用动态内存分配，因为堆也需要本函数。
void bsp::di::task::ResumeAllTask()
{
	xTaskResumeAll();
}
